...
File: STPMDP_ORS.jl
---------------
This file contains the implementation of the MDP for the triage/transport problem. ORS Routing is used. 
=#
...
# Function to compute travel time using ORS
function calculate_travel_time(loc1::Location, loc2::Location)
    # Estimate the distance first
    ...
    if response.status == 200
        ...
        # Adjusted according to the provided output format
        ...
        println("Failed to get travel time from ORS API: HTTP status $(response.status)")
...
# Forward search function for solving for optimal policy
...
# Route using current CA policy (i.e., nearest hospital)
...
#Function for sampling a random stroke type using population statistics
